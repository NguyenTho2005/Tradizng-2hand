...
document.addEventListener("DOMContentLoaded", () => {
    ...
    fileInput.type = "file";
    fileInput.accept = "image/*,video/*"; // Hỗ trợ cả ảnh và video
    fileInput.multiple = true; // Hỗ trợ chọn nhiều tệp

    // Xử lý khi nhấn vào box upload
    ...
    fileInput.addEventListener("change", () => {
        ...
        // Xóa các nội dung xem trước cũ nếu có
        uploadContainer.querySelectorAll(".preview-item").forEach(item => item.remove());
        ...
        Array.from(files).forEach(file => {
            ...
            if (file.type.startsWith("image/")) {
                ...
                video.controls = true; // Bật điều khiển cho video
                previewItem.appendChild(video);
            }

            // Thêm xem trước vào container
            uploadContainer.appendChild(previewItem);
        });
    });
});
